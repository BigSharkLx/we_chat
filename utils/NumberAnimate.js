...
class NumberAnimate {
    ...
    constructor(opt) {
        ...
        this.tempValue = this.opt.fromNum;//累加变量值
this.opt.refreshTime=Math.ceil(this.opt.speed/(this.opt.toNum-this.opt.fromNum));
        this.loopCount = 0;//循环次数计数

        this.loops = Math.ceil(this.opt.toNum-this.opt.fromNum);//数字累加次数

        this.increment = ((this.opt.toNum-this.opt.fromNum)/this.loops);//每次累加的值
